...
async function checkEnvironmentVariables() {
    try {
        ...
        if (missingVars.length > 0) {
            ...
            logger.info('💡 אנא העתק את .example ל- ומלא את הערכים החסרים');
...
async function runHealthChecks() {
    try {
        ...
        try {
            ...
            const client = new openai({ apiKey: process.OPENAI_API_KEY });
...
async function printStartupInfo() {
    try {
        ...
        logger.info(`🔗 כתובת המערכת: http://localhost:${process.PORT || 3000}`);
