...
const { getTasks, getDebts, getBureaucracy, initSupabase } = require('./config/supabase');
...
const GMAIL_CLIENT_ID = process.env.GOOGLE_CLIENT_ID || 'your_google_client_id_here';
const GMAIL_CLIENT_SECRET = process.env.GOOGLE_CLIENT_SECRET || 'your_google_client_secret_here';
const GMAIL_REDIRECT_URI = process.env.GOOGLE_REDIRECT_URI || 'http://localhost:3000/auth/google/callback';
...
async function loadDataFromSupabase() {
    try {
        ...
        console.log('🔍 קורא נתונים מ-Supabase...');
        const [tasksResult, debtsResult, bureaucracyResult] = await Promise.all([
            getTasks(1),
            getDebts(1),
            getBureaucracy(1)
        ]);
        
        console.log('📊 תוצאות Supabase:', {
            tasks: tasksResult.data?.length || 0,
            debts: debtsResult.data?.length || 0,
            bureaucracy: bureaucracyResult.data?.length || 0,
            tasksError: tasksResult.error,
            debtsError: debtsResult.error,
            bureaucracyError: bureaucracyResult.error
        });
        
        const data = {
            tasks: tasksResult.data || [],
            debts: debtsResult.data || [],
            bureaucracy: bureaucracyResult.data || [],
            clients: [], // Add clients if needed
            stats: {
                total: (tasksResult.data?.length || 0) + (debtsResult.data?.length || 0) + (bureaucracyResult.data?.length || 0),
                urgent: 0,
                pending: 0,
                in_progress: 0,
                done: 0
        ...
        data.tasks.forEach(task => {
            if (task.status === 'urgent' || task.priority === 'דחוף') data.stats.urgent++;
            else if (task.status === 'pending' || task.status === 'בהמתנה') data.stats.pending++;
            else if (task.status === 'in_progress' || task.status === 'בעבודה') data.stats.in_progress++;
            else if (task.status === 'done' || task.status === 'הושלם') data.stats.done++;
        });
        
        return data;
    } catch (error) {
        console.error('❌ שגיאה בטעינת נתונים מ-Supabase:', error);
        ...
        return {
            ...
            bureaucracy: [],
            clients: [],
            stats: { total: 0, urgent: 0, pending: 0, in_progress: 0, done: 0 }
        };
    }
}

loadDataFromSupabase().then(async data => {
    realData = data;
    console.log('📊 נתונים אמיתיים נטענו מ-Supabase:', data.stats);
...
app.get('/api/gmail/auth', (req, res) => {
    ...
    res.json({ 
        success: true, 
            ...
            instructions: 'כדי להשתמש ב-Gmail אמיתי, הגדר GOOGLE_CLIENT_ID ב-'
        });
    } else {
...
`;
        ...
        try {
            response = await aiService.analyzeEmail(prompt);
        ...
        console.error('AI email analysis error:', aiError);
